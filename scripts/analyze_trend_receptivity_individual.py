...
def analyze_trend_receptivity_change_full():
    ...
    with engine.connect() as connection:
        ...
        df_report['change'] = df_report['final_receptivity'] - df_report['initial_receptivity']
        ...
        # 6. Analyze individual agents for 'SpiritualMentor'
        print("\n--- Individual Trend Receptivity Changes for SpiritualMentor ---")
        spiritual_mentor_agents = df_report[df_report['profession'] == 'SpiritualMentor']
        if not spiritual_mentor_agents.empty:
            print(spiritual_mentor_agents[['person_id', 'initial_receptivity', 'final_receptivity', 'change']].to_string(index=False))
            avg_change_spiritual_mentor = profession_stats[profession_stats['profession'] == 'SpiritualMentor']['change'].iloc[0]
            print(f"\nAverage change for SpiritualMentor profession: {avg_change_spiritual_mentor:.6f}")
        else:
            print("No SpiritualMentor agents found in this simulation.")
